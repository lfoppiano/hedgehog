...
Cette femme, très simple, est vraiment épatante. Elle m'explique que depuis le début elle écoute les informations de la radio anglaise et les diffuse dans le quartier. Je leur demande s'ils sont affiliés à une organisation — Oui - Laqelle "la résistance " C'sst ici que parle le bon sens et la clairvoyance : au sommet on se bat pour des initiales, à la base on croit en la résistance.
...
if 'entities' in nerdResponse:
    ...
    for entity in nerdResponse['entities']:
        ...
        if 'wikipediaExternalRef' in entity:
            concept, conceptStatus = nerdClient.fetchConcept(entity['wikipediaExternalRef'], lang)
