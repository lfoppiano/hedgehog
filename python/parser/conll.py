...
class DependencyTree(nx.DiGraph):
    ...
    def sentence_plus_word(self, n, attr_dict={}, head=None, edge_attribs={}):
        ...
        multiwordtopropagate = []
        # the trailing pppp nodes
        for j in self.nodes()[n + 1:]:
            adjusted_node = j + 1
            original_head = self.head_of(j)
            if j in self.graph["multi_tokens"]:
                multiwordtopropagate.append(j)

            # print(j,self.graph["multi_tokens"])
    ...
    def sentence_minus_word(self, n,
                            propagate_edges=False):  # notice we are not using remove_node because we need to recalculate node indices and so on
    ...
    def filter_sentence_content(self, replace_subtokens_with_fused_forms=False, lang=None, posPreferenceDict=None,
                                node_properties_to_remove=None, remove_deprel_suffixes=False,
...
class CoNLLReader(object):
    ...
    "" "Static properties"""
    CONLL06_COLUMNS = [('id', int), ('form', str), ('lemma', str), ('cpostag', str), ('postag', str), ('feats', str),
                       ('head', int), ('deprel', str), ('phead', str), ('pdeprel', str)]
    # CONLL06_COLUMNS = ['id', 'form', 'lemma', 'cpostag', 'postag', 'feats', 'head', 'deprel', 'phead', 'pdeprel']
    CONLL06DENSE_COLUMNS = [('id', int), ('form', str), ('lemma', str), ('cpostag', str), ('postag', str),
                            ('feats', str), ('head', int), ('deprel', str), ('edgew', str)]
    ...
    def write_conll(self, list_of_graphs, conll_path, conllformat='conllu', print_fused_forms=False,
                    print_comments=True):
    ...
    def read_conll_u_filename(self, filename, keepFusedForm=False, lang=None, posPreferenceDict=None):
        return self.read_conll_u(open(filename).readlines(), keepFusedForm, lang, posPreferenceDict)

    def read_conll_u(self, lines, keepFusedForm=False, lang=None, posPreferenceDict=None):
        sentences = []
        sent = DependencyTree()
        multi_tokens = {}
        ...
        for line_no, line in enumerate(lines):
            ...
            if not line:
                ...
                if len(parts) != len(self.CONLL_U_COLUMNS):
                    error_msg = 'Invalid number of columns in line {} (found {}, expected {})'.format(line_no,
                                                                                                      len(parts), len(
                            self.CONLL_U_COLUMNS))
